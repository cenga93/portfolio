...
gulp.task("sass", function () {
  ...
  if (config.debug === false) {
    ...
    stream.pipe(concat("main.min.css")).pipe(gulp.dest(config.path.distPath + "css/"));
  }
...
gulp.task("criticalSass", function () {
  let stream = gulp
    .src(["./src/scss/critical.scss", "./src/scss/critical/*.scss"], {
      allowEmpty: false,
    })
    .pipe(plumber())
    .pipe(bulkSass())
  ...
  if (config.debug === false) {
    ...
    stream.pipe(concat("critical.min.css")).pipe(gulp.dest(config.path.distPath + "css/"));
  }
...
gulp.task("criticalJs", function () {
  let stream = gulp
    .src(["./src/js/critical/*", "./src/js/critical.js"], {
      allowEmpty: true,
    })
    .pipe(plumber())
    .pipe(
 ...
 * Returns void
 */
gulp.task("svg", gulp.series("sassvg", "svgsprite", "sass", "criticalSass"));
...
gulp.task("watch", function (cb) {
  ...
  gulp.watch(["./src/js/main.js"], gulp.series("js")).on("change", browserSync.reload);

  // Watch all critical JS files
  gulp.watch(["./src/js/critical.js"], gulp.series("criticalJs")).on("change", browserSync.reload);

  // Watch SVG folders
  gulp.watch(["./src/svg/**/*.svg"], gulp.series("sassvg")).on("change", browserSync.reload);
